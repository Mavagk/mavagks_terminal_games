...
impl FloatSuppliedFunction {
	...
	pub fn get_help_info(self) -> (&'static str, &'static str, &'static str, &'static str, &'static [&'static str]) {
		match self {
			...
			&["SIN(X) = COS(X) * TAN(X)", "SIN(X) = 1 / CSC(X)", "SIN(ASIN(X)) = X if -1 <= X <= 1",
			"SIN(X) = COS(PI / 2 - X)", "SIN(0) = 0", "SIN(PI / 6) = 1 / 2", "SIN(PI / 4) = SQR(2) / 2", "SIN(PI / 3) = SQR(3) / 2", "SIN(PI / 2) = 1",
			...
			"The angle will be in radians by default but can be set to degrees, gradians or turns by setting OPTION ANGLE to DEGREES, GRADIANS or TURNS respectively.",
			&["TAN(X) = SIN(X) / COS(X)", "TAN(X) = 1 / COT(X)", "TAN(ATAN(X)) = X",
			"TAN(0) = 0", "TAN(PI / 6) = SQR(3) / 3", "TAN(PI / 4) = 1", "TAN(PI / 3) = SQR(3)",
			"TAN(X + 2 * PI) = TAN(X)"]),
			Self::Cot => ("COT(X)", "Cotangent", "Gives the the adjacent to opposite lengths ratio of a right triangle given a non-right angle.",
			"The angle will be in radians by default but can be set to degrees, gradians or turns by setting OPTION ANGLE to DEGREES, GRADIANS or TURNS respectively.",
			&["COT(X) = COS(X) / SIN(X)", "COT(X) = 1 / TAN(X)", "COT(ACOT(X)) = X"]),
			Self::Sec => ("SEC(X)", "Secant", "Gives the the hypotenuse to adjacent lengths ratio of a right triangle given a non-right angle.",
			"The angle will be in radians by default but can be set to degrees, gradians or turns by setting OPTION ANGLE to DEGREES, GRADIANS or TURNS respectively.",
			&["SEC(X) = TAN(X) / SIN(X)", "SEC(X) = 1 / COS(X)", "SEC(ASEC(X)) = X if X <= -1 or X >= 1"]),
